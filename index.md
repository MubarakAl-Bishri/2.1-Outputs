...
<ul>
  ...
  {% else %}
    {% for item in site.pages %} {% if item.title and item.url != "/" and item.layout != nil and item.published != false and item.path != "README.md" %}
        {% assign item_url = item.url | relative_url %}
        {% unless _seen_urls contains item_url %}
          
          <li dir="{{ item.dir | default: 'auto' }}">
